...
class BinarySearchTree {
  ...
  // part of the REQUIRES clause for all member functions - 
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    ...
    //find both left and right heights and use the longest
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    if (node == nullptr) {
      ...
      //copy all parts of node
      Node *copy = new Node(node->datum, node->left, node->right);
  ...
  //           in the tree rooted at 'node' or a null pointer if the tree
  // is empty.
  // NOTE: This function must be tail recursive.
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if(node == nullptr) {
      ...
      bool right = false;
      //nothing else
      if (min_element_impl(node->right) == nullptr){
        right = true;
      }
      //new lowest
      else {
        right = less(node->datum, min_element_impl(node->right)->datum);
      }
      //close to root
      if (max_element_impl(node->left) == nullptr){
        left = true;
      }
      //far from root
      else {
        left = less(max_element_impl(node->left)->datum, node->datum);
      }
      return check_sorting_invariant_impl(node->left, less) && 
        check_sorting_invariant_impl(node -> right, less) && (left && right);
    }
  }
